defmodule Triton.CQL.Delete do
  ...
  defp where_fragment({k, v}) when is_list(v) do
    v |> Enum.map(fn {c, v} -> where_fragment({k, c, v}) end)
  end

  defp where_fragment({k, v}), do: ["#{k} = " <> value(v)]
  defp where_fragment({k, :in, v}) do
    "#{k} IN (" <> Enum.map_join(v, ", ", fn v -> value(v) end) <> ")"
  end
  defp where_fragment({k, c, v}), do: "#{k} #{c} " <> value(v)

  defp constrain(constraints) when is_list(constraints) do
    " IF " <> Enum.map_join(constraints, ", ", fn {k, v} -> "#{k} = " <> value(v) end)
  end
  defp constrain(_), do: ""
